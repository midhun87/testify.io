...
app.get('/api/public/certificate/:certId', async (req, res) => {
    try {
        ...
        console.error("CRITICAL AWS ERROR fetching certificate:", err);
        
        // Expose the specific AWS error to the frontend for production debugging
        const errorMessage = err.name === 'AccessDeniedException' 
            ? "AWS IAM Error: The EC2 instance does not have permission to read TestifyCertificates." 
