...
const S3_BUCKET_NAME = "hirewithusjobapplications1"; 
...
const kinesisVideoClient = new KinesisVideoClient({
    ...
    credentials: {
        accessKeyId: 'AKIAT4YSUMZD5BYSCRAY', // Your existing key
        secretAccessKey: process.env.CHIME_AWS_SECRET_ACCESS_KEY || '4brN5BSSPFXywX3SMameT7h065Xm3+Ph0NQui5y+' // Your existing secret
    }
});
...
const s3Config = new S3Client({
    region: process.env.AWS_REGION || 'ap-south-1',
    credentials: {
        accessKeyId: process.env.AWS_ACCESS_KEY_ID || 'AKIA5FK6RJPRK5RSVOEA',
        secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY || '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'
    }
});
...
async function compileWithCustomCompiler(language, code, input) {
    ...
    const compilerUrl = 'http://65.2.104.225:8000';
...
// --- CLOUDINARY CONFIG ---
cloudinary.config({
  cloud_name: 'dzuxwvamy',
  api_key: '784235885441981',
  api_secret: '63de4Rp1YscYf1cT6vDcHdnTuXg'
});
...
const rekognitionClient = new RekognitionClient({
    region: 'ap-south-1',
    credentials: {
        accessKeyId: 'AKIA5FK6RJPRK5RSVOEA',
        secretAccessKey: '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'
    }
});

app.use(cors());
// Increase payload limit to 50MB to handle multiple file uploads
...
Here is the text:\n\n${text}`;
        ...
        const apiKey = 'AIzaSyCJ9PD5WSXvG56IhXvb38yL2vL7TIzbhOo';
...
// =================================================================
const courseMaterialsUpload = multer({
    storage: multerS3({
        s3: s3Config,
        bucket: 'hirewithusjobapplications1', // Target bucket specified
        //acl: 'public-read', // Assuming files need to be publicly readable by students. Adjust if using signed URLs.
        ...
        key: function (req, file, cb) {
            ...
            const uniqueName = `course-materials/${uuidv4()}-${file.originalname.replace(/\s+/g, '-')}`;
            cb(null, uniqueName);
        }
    }),
...
app.post('/api/admin/upload', authMiddleware, courseMaterialsUpload.single('file'), async (req, res) => {
    // Basic role check
    if (req.user.role !== 'Admin') return res.status(403).json({ message: 'Access denied.' });
    ...
    try {
        // req.file.location is populated by multer-s3 with the public URL
        res.status(200).json({ url: req.file.location });
    } catch (error) {
        console.error("Upload Route Error:", error);
        res.status(500).json({ message: 'Error returning file URL.' });
    }
});

// --- UPDATED ENDPOINT: CREATE COURSE ---
app.post('/api/admin/courses', authMiddleware, async (req, res) => {
    ...
    // Map data from the new Quill/Attachment frontend payload
    const processedModules = modules.map(module => ({
        ...
        subModules: module.subModules.map(subModule => ({
            ...
            subModuleId: uuidv4(),
            // New rich text payload containing HTML
            contentHtml: subModule.contentHtml || '', 
            // New Array containing { name, url, type } for PDFs, PPTs, Videos
...
// --- UPDATED ENDPOINT: GET ALL COURSES ---
...
// --- UPDATED ENDPOINT: GET SINGLE COURSE (For Edit Page) ---
...
// --- UPDATED ENDPOINT: UPDATE COURSE ---
app.put('/api/admin/courses/:id', authMiddleware, async (req, res) => {
    ...
    // Map the new payload structures cleanly, keeping existing IDs if provided
    const processedModules = modules.map(module => ({
        ...module,
    ...
    try {
        const { Item: existingCourse } = await docClient.send(new GetCommand({ 
            TableName: "TestifyCourses", 
            Key: { courseId } 
        }));
...
app.get('/api/student/dashboard-data', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const history = allHistory.filter(r => {
            ...
            // --- FIX IS HERE: Include in dashboard stats if preview is allowed ---
            return testDetails && (testDetails.resultsPublished === true || r.allowPreview === true);
        });
        ...
        // --- Fetch Compiler/CodeLab Stats ---
        const compilerScoresResponse = await docClient.send(new ScanCommand({
            TableName: "TestifyCompilerScores", 
            FilterExpression: "studentEmail = :email AND attribute_not_exists(recordType)", 
            ExpressionAttributeValues: {
                ":email": studentEmail
            }
        }));
...
app.get('/api/student/history', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const enrichedHistory = results.map(result => {
            ...
            // --- FIX IS HERE: Check if resultsPublished is true OR allowPreview is true ---
            if (testDetails && (testDetails.resultsPublished === true || result.allowPreview === true)) {
                return {
                    ...result,
...
Here is the text:\n\n${text}`;
        ...
        const apiKey = process.env.GEMINI_API_KEY || 'AIzaSyCJ9PD5WSXvG56IhXvb38yL2vL7TIzbhOo';
...
app.post('/api/public/submit-coding-test', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (!assignment || assignment.studentEmail !== candidateEmail || assignment.testType !== 'coding') {
            return res.status(403).json({ message: 'Assignment validation failed.' });
        }

        const jobId = assignment.jobId || null;

        // 2. Fetch Coding Test Definition
        const { Item: test } = await docClient.send(new GetCommand({ 
            TableName: HIRING_CODING_TESTS_TABLE, 
        ...
        if (!test) {
            return res.status(404).json({ message: "Coding test definition not found." });
        }

        // 3. Create Problem Map for Scoring
        const problemMapForScores = new Map();
        let totalPossibleMarks = 0;

        const sections = test.sections || (test.problems ? [{ title: "Coding Problems", problems: test.problems }] : []);
        
        sections.forEach(section => {
            section.problems.forEach(p => {
                const score = parseInt(p.score, 10) || 0;
                // Store using both ID variants to ensure matching works
                problemMapForScores.set(p.problemId || p.id, { 
                    score: score, 
                    title: p.title, 
        ...
        const detailedSubmissions = submissions.map(sub => {
            ...
            // --- CRITICAL CHANGE: Trust the evaluationResults from frontend ---
            // The frontend has already run the code and determined pass/fail for each case.
            const evaluationResults = sub.evaluationResults || [];
            
            const passedCases = evaluationResults.filter(r => {
        ...
        const newSubmissionResult = {
            ...
            rollNumber: candidateDetails.rollNumber || 'N/A',
            collegeName: candidateDetails.collegeName || 'N/A',
            department: candidateDetails.department || 'N/A',
            // --- FIX: Added profileImageUrl here ---
            profileImageUrl: candidateDetails.profileImageUrl || null, 
            testTitle: test.title || 'N/A',
            submissions: detailedSubmissions,
            score: totalScore || 0,
            totalMarks: totalPossibleMarks || 0,
            result: (totalPossibleMarks > 0) 
                ? ((totalScore / totalPossibleMarks) * 100 >= (test.passingPercentage || 50) ? "Pass" : "Fail")
                : "N/A",
            submittedAt: new Date().toISOString(),
            violationReason: violationReason || null
        };
        ...
        console.error(`[SUBMIT CODING TEST] FATAL ERROR:`, error);
        res.status(500).json({ message: 'Server error submitting test.' });
    }
});
...
// EXTERNAL CANDIDATE: Get test details using token
// EXTERNAL CANDIDATE: Get test details using token
// --- REPLACE THE ENTIRE /api/public/test-details ENDPOINT WITH THIS ---
...
app.get('/api/public/test-details', authMiddleware, async (req, res) => {
    try {
        ...
        const { testId: testPkFromToken, email: candidateEmail, assignmentId, isMockTest } = req.user;
        ...
        // 3. Validation (Skipped for Mock Tests)
        if (!isMockTest) {
            ...
            // *** CRITICAL: TIME CHECK REMOVED TO FIX YOUR ERROR ***
            // The code that checked (now < startTime) is deleted.
           ...
           if (existing && existing.length > 0) {
               return res.status(403).json({ message: 'This test link has already been used.' });
           }
        }
        
        // 4. Fetch the Actual Test Questions
        let test;
        const type = assignment ? assignment.testType : req.user.testType;
        
        if (type === 'coding') {
            const { Item } = await docClient.send(new GetCommand({ TableName: HIRING_CODING_TESTS_TABLE, Key: { codingTestId: testPkFromToken } }));
            test = Item;
        } else if (type === 'aptitude') {
            const { Item } = await docClient.send(new GetCommand({ TableName: HIRING_APTITUDE_TESTS_TABLE, Key: { aptitudeTestId: testPkFromToken } }));
            test = Item;
        }
        
        if (!test) return res.status(404).json({ message: 'Test content not found.' });
...
Here is the text to analyze:\n\n${text}`;
        ...
        const apiKey = process.env.GEMINI_API_KEY || 'AIzaSyCJ9PD5WSXvG56IhXvb38yL2vL7TIzbhOo'; // It's better to use environment variables
        if (!apiKey) {
...
Here is the text:\n\n${text}`;
        ...
        const apiKey = 'AIzaSyCJ9PD5WSXvG56IhXvb38yL2vL7TIzbhOo'; // Use process.env in production
...
app.post('/api/student/register', async (req, res) => {
    ...
    try {
        ...
        // FIX: Check for existing user in "TestifyUsers"
        const existingUser = await docClient.send(new GetCommand({ 
            TableName: "TestifyUsers", 
            Key: { email: emailLower } 
        }));
        ...
        // FIX: Put user into "TestifyUsers"
        await docClient.send(new PutCommand({ 
            TableName: "TestifyUsers", 
            Item: newUser 
        }));
 ...
 * @desc Student: Fetch eligible job openings based on college
 * @access Private (Student)
 */
app.get('/api/student/jobs', studentAuthMiddleware, async (req, res) => {
    ...
    try {
        // 1. Scan for all currently open jobs (Deadline has not passed)
        const { Items: allJobs } = await docClient.send(new ScanCommand({ 
            TableName: HIRING_JOBS_TABLE, // Defined as "HiringJobs"
            FilterExpression: "applicationDeadline > :now", 
            ExpressionAttributeValues: { ":now": nowISO } 
        }));

        if (!allJobs || allJobs.length === 0) {
            console.log(`[STUDENT_JOBS_INFO] No open jobs found in total.`);
            return res.json([]);
        }

        // 2. Filter jobs based on student's college eligibility
        const eligibleJobs = allJobs.filter(job => 
            job.eligibleColleges && 
            Array.isArray(job.eligibleColleges) && 
            job.eligibleColleges.includes(studentCollege)
        );

        console.log(`[STUDENT_JOBS_FILTER] Found ${eligibleJobs.length} eligible jobs.`);

        if (eligibleJobs.length === 0) {
            return res.json([]);
        }

        // --- [FIXED SECTION FOR MODERATOR NAMES] ---
        ...
        if (moderatorEmails.length > 0) {
            ...
            const batchGetParams = {
                RequestItems: {
                    "TestifyUsers": { // FIX: Target the admin/moderator user table
                        Keys: keys, 
                        ProjectionExpression: "email, fullName" 
                    }
                }
            ...
            // FIX: Read from "TestifyUsers" response
        ...
        // 5. Format jobs with the correct 'postedBy' name
        const formattedJobs = eligibleJobs.map(job => ({
            ...job,
            postedBy: moderatorMap.get(job.createdBy) || 'Company'
        }));

        // 6. Sort jobs by creation date (newest first)
        formattedJobs.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

        console.log(`[STUDENT_JOBS_SUCCESS] Returning ${formattedJobs.length} jobs.`);
        res.json(formattedJobs);

    } catch (error) {
        console.error(`[STUDENT_JOBS_FATAL_ERROR] Failed fetching jobs for student ${req.user.email}:`, error);
        if (!res.headersSent) {
            res.status(500).json({ message: 'Server error fetching jobs. Please try again later.' });
        }
    }
});
app.get('/api/student/applications', studentAuthMiddleware, async (req, res) => {
...
app.post('/api/public/save-code-snippet', authMiddleware, async (req, res) => {
    // Determine IDs based on user type (External vs Internal)
    const assignmentIdFromToken = req.user?.assignmentId;
    ...
    // Destructure body
    const { assignmentId, testId, problemId, code, language, evaluationResults } = req.body;

    console.log('[SAVE SNIPPET] Request received.', { problemId, language });
    ...
    // Use token IDs if external, otherwise body IDs
    ...
    if (!effectiveAssignmentId || !effectiveTestId || !effectiveEmail) {
        return res.status(400).json({ message: 'Could not determine required identifiers.' });
    }

    // --- Generate snippetId (Partition Key) ---
    // Using a composite key to ensure one snippet per problem per assignment
    const snippetId = `snip_${effectiveAssignmentId}_${problemId}`;
    const primaryKey = { snippetId: snippetId };

    try {
        // --- Prepare full item data ---
        // We include evaluationResults here so the state is fully persisted
        const snippetData = {
            snippetId: snippetId,
            ...
            code: code,
            language: language,
            evaluationResults: evaluationResults || [], // Save test case results
        ...
        if (existingSnippet) {
            ...
            await docClient.send(new UpdateCommand({
                TableName: HIRING_CODE_SNIPPETS_TABLE,
                Key: primaryKey,
                UpdateExpression: "SET #c = :c, #l = :l, #er = :er, #ts = :ts, #tid = :tid, #email = :email, #aid = :aid, #pid = :pid",
                ExpressionAttributeNames: {
                    "#c": "code",
                    "#l": "language",
                    "#er": "evaluationResults", // New field
                    "#ts": "savedAt",
                    "#tid": "testId",
                    "#email": "userEmail",
                    "#aid": "assignmentId",
            ...
            res.status(200).json({ message: 'Code snippet and results updated successfully.' });
            ...
            res.status(201).json({ message: 'Code snippet and results saved successfully.' });
        }
...
// Add this endpoint to your backend.js file 
// Ensure your .env file has GEMINI_API_KEY=your_actual_api_key_here

app.post('/generate-ai-test', async (req, res) => {
    try {
        ...
        const prompt = `Generate ${count} ${type} questions about the topic: "${topic}". 
        Return the response as a strict JSON object with a single key "questions" containing an array of objects. 
        Each object must have: 
        1. "questionText": The question string.
        2. "options": An array of exactly 4 strings.
        3. "correctAnswer": The index (0-3) of the correct option.
...
// =================================================================
// --- NEW REPORTING ENDPOINTS FOR ADMIN (INDIVIDUAL & BULK ZIP) ---
// =================================================================

// ADMIN: Get detailed test preview (Full report) for a specific result
app.get('/api/admin/test-preview/:resultId', authMiddleware, async (req, res) => {
    if (req.user.role !== 'Admin') {
        return res.status(403).json({ message: 'Access denied.' });
    }
    const { resultId } = req.params;
    ...
    try {
        ...
        // 2. Fetch original test (or use embedded static data)
        let test;
        if (result.staticTestData) {
            test = result.staticTestData;
        } else {
        ...
        if (!test) return res.status(404).json({ message: 'Original test content not found.' });

        // 3. Send combined data
        res.json({
            studentEmail: result.studentEmail,
            studentName: result.studentName || result.studentEmail,
            score: result.score,
        ...
        console.error("Admin Get Test Preview Error:", error);
        res.status(500).json({ message: 'Server error fetching test preview.' });
    }
});

// ADMIN: Get bulk detailed test previews for ZIP generation
app.post('/api/admin/bulk-test-previews', authMiddleware, async (req, res) => {
    if (req.user.role !== 'Admin') {
        return res.status(403).json({ message: 'Access denied.' });
    }
    ...
    if (!resultIds || !Array.isArray(resultIds)) {
        return res.status(400).json({ message: 'An array of resultIds is required.' });
    }

    try {
        const reports = [];
        
        // Loop through each requested result ID
        for (const resultId of resultIds) {
            const { Item: result } = await docClient.send(new GetCommand({ 
                TableName: "TestifyResults", Key: { resultId } 
            }));
            if (!result) continue;

            let test = result.staticTestData;
            if (!test) {
                const { Item } = await docClient.send(new GetCommand({ 
                    TableName: "TestifyTests", Key: { testId: result.testId } 
                }));
                test = Item;
            }
        ...
        console.error("Bulk Test Previews Error:", error);
        res.status(500).json({ message: 'Server error generating bulk reports.' });
    }
});
// REPLACE the previous /api/public/reports/:email route in your backend(31).js WITH THIS ONE

// ADD THIS ROUTE TO YOUR backend(31).js FILE
// This handles the request made when someone scans the ID card QR code.
// ADD THIS ROUTE TO YOUR backend(31).js FILE
// This handles the request made when someone scans the ID card QR code.

app.get('/api/public/reports/:email', async (req, res) => {
    try {
        ...
        // Sort by newest first to extract the most up-to-date user info
        Items.sort((a, b) => new Date(b.submittedAt) - new Date(a.submittedAt));
        const latestResult = Items[0];

        // Extract student details
        const studentName = latestResult.studentName || studentEmail.split('@')[0];
        const college = latestResult.college || latestResult.university || "College not specified";
        const profileImageUrl = latestResult.profileImageUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(studentName)}&background=4f46e5&color=fff`;

        // Map the results to include total, coding, codespace, and sql scores
        const publicReports = Items.map(item => {
            
            // Extract the specific scores dynamically (Pass null if they don't exist for this specific test)
            let codingScore = (item.codingScore !== undefined) ? item.codingScore : (item.result && item.result.codingScore !== undefined ? item.result.codingScore : null);
            let codeSpaceScore = (item.codeSpaceScore !== undefined) ? item.codeSpaceScore : (item.result && item.result.codeSpaceScore !== undefined ? item.result.codeSpaceScore : null);
            let sqlScore = (item.sqlScore !== undefined) ? item.sqlScore : (item.result && item.result.sqlScore !== undefined ? item.result.sqlScore : null);

            return {
                testId: item.testId,
                testTitle: item.staticTestData?.title || 'General Assessment',
                totalScore: item.score || 0,
                codingScore: codingScore,
                codeSpaceScore: codeSpaceScore,
                sqlScore: sqlScore,
                passingScore: item.staticTestData?.passingMarks || 40,
        ...
        console.error("Error fetching public QR reports:", error);
...
////////////////////////////////////////////////////////////////////////////////
app.post('/api/sme/public-tests', smeAuth, async (req, res) => {
    try {
        const { type, title, domain, skills, description, sections } = req.body;
        ...
        await docClient.send(new PutCommand({
            TableName: "TestifyPublicTests",
            Item: newTest
        }));

        res.status(201).json({ message: "Public test published successfully", testId });
    } catch (error) {
...
app.get('/api/public/tests', async (req, res) => {
    try {
        ...
        // CRITICAL: Sanitize data - Remove correct answers before sending to frontend
        const sanitizedTests = Items.map(test => {
            const safeSections = test.sections.map(section => ({
                ...section,
                questions: section.questions.map(q => {
                    const { correct, correctAnswers, ...safeQuestion } = q;
                    return safeQuestion; 
                })
            }));

            return { ...test, sections: safeSections };
        });

        const practice = sanitizedTests.filter(t => t.type === 'practice');
        const mock = sanitizedTests.filter(t => t.type === 'mock');

        res.json({ practice, mock });
    } catch (error) {
...
app.post('/api/public/practice/submit', async (req, res) => {
    try {
        const { testId, userEmail, userName, userMobile, userOrg, answers } = req.body;
        ...
        test.sections.forEach((section, secIdx) => {
            section.questions.forEach((q, qIdx) => {
                totalQuestions++;
                const userAnswer = answers[`${secIdx}_${qIdx}`]; 
                ...
                if (q.type === 'single' && userAnswer === q.correct) {
                    ...
                    if (q.correctAnswers.includes(userAnswer?.toLowerCase().trim())) {
                        correctAnswersCount++;
                    }
                }
            });
        ...
        const scorePercentage = totalQuestions === 0 ? 0 : (correctAnswersCount / totalQuestions) * 100;
        const resultId = uuidv4();

        // Save the result to DynamoDB WITH the new fields
        await docClient.send(new PutCommand({
            TableName: "TestifyPublicResults",
            Item: { 
                resultId, 
                testId, 
                userEmail, 
                userName, 
                userMobile: userMobile || 'N/A', // NEW
                collegeName: userOrg || 'N/A',   // NEW (Mapped to collegeName for SME consistency)
        ...
        res.json({ message: "Evaluation complete", score: scorePercentage.toFixed(2), resultId });
    } catch (error) {
...
// --- 4. MOCK REGISTER ---
// --- 4. MOCK REGISTER ---
app.post('/api/public/mock/register', async (req, res) => {
    try {
        const { email, name, mobile, org, testId } = req.body;
        const voucherCode = crypto.randomBytes(4).toString('hex').toUpperCase();

        await docClient.send(new PutCommand({
            TableName: "TestifyVouchers",
            Item: {
                voucherCode,
                assignedEmail: email,
                candidateName: name,      // NEW
                candidateMobile: mobile,  // NEW
                candidateOrg: org,        // NEW
                testId,
                status: "ISSUED",
                createdAt: new Date().toISOString()
            }
                ...
                <tr>
                    <td align="center">
                        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                            <!-- Header with Logo -->
                            <tr>
                                <td align="center" style="padding: 30px 20px; border-bottom: 1px solid #edf2f7;">
                                    <img src="https://res.cloudinary.com/dzuxwvamy/image/upload/v1780483593/Credex_szkzcq.png" alt="Credex Logo" style="height: 40px; width: auto; display: block;">
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td style="padding: 40px 30px;">
                                    ...
                                    <p style="font-size: 16px; color: #475569; line-height: 1.6; margin-bottom: 30px;">
                                        Thank you for registering for the premium mock test. You're one step closer to validating your expertise!
                                    </p>
                                    ...
                                    <div style="background-color: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 8px; padding: 20px; text-align: center; margin: 30px 0;">
                                        <p style="margin: 0 0 8px 0; font-size: 14px; color: #64748b; text-transform: uppercase; letter-spacing: 1px;">Your Exclusive Voucher Code</p>
                                        <p style="margin: 0; font-size: 32px; font-weight: 800; color: #4f46e5; letter-spacing: 4px;">${voucherCode}</p>
                                    </div>
                                    
                                    <p style="font-size: 16px; color: #475569; line-height: 1.6; text-align: center; margin-bottom: 35px;">
                                        Please click the button below or enter your code on the portal to schedule your exam time.
                                    </p>
                                    ...
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                                        <tr>
                                            <td align="center">
                                                <a href="https://testify.xetasolutions.in/public-portal.html#schedule" style="background-color: #4f46e5; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: bold; font-size: 16px; display: inline-block;">Schedule Exam Now</a>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                            ...
                            <!-- Footer -->
                            <tr>
                                <td align="center" style="background-color: #f8fafc; padding: 24px; border-top: 1px solid #edf2f7;">
                                    <p style="margin: 0; font-size: 14px; color: #94a3b8; line-height: 1.5;">
                                        Best regards,<br>
                                        <strong style="color: #64748b;">Credx by Testify</strong>
                                    </p>
                                </td>
                            </tr>
                        </table>
        ...
        const emailParams = {
            FromEmailAddress: '"Credex - TESTIFY" <support@testify-lac.com>', 
            Destination: { ToAddresses: [email] },
            Content: {
                Simple: {
                    Subject: { Data: "Your Credex Test Voucher" },
                    Body: { 
                        Html: { Data: htmlContent, Charset: "UTF-8" },
                        Text: { Data: `Hello ${name},\n\nThank you for registering for the premium mock test.\n\nYour exclusive voucher code is: ${voucherCode}\n\nPlease click the link below to schedule your exam time:\nhttps://testify.xetasolutions.in/public-portal.html#schedule\n\nBest regards,\nHire With Us Team`, Charset: "UTF-8" }
                    }
                }
            }
        };
        ...
        res.json({ message: "Voucher generated and emailed successfully." });
    } catch (error) {
        console.error("Voucher generation/email error:", error);
        res.status(500).json({ message: "Failed to generate test voucher or send email." });
    }
});
// --- 5. MOCK SCHEDULE ---
// ... existing code ...
// --- 5. MOCK SCHEDULE ---
app.post('/api/public/mock/schedule', async (req, res) => {
    try {
        const { email, voucherCode, scheduledTime } = req.body;

        const { Item: voucher } = await docClient.send(new GetCommand({
            TableName: "TestifyVouchers",
            Key: { voucherCode }
        }));

        if (!voucher || voucher.assignedEmail !== email) {
            return res.status(400).json({ message: "Invalid voucher code or email mismatch." });
        }

        if (voucher.status !== "ISSUED") {
            return res.status(400).json({ message: "This voucher has already been utilized or scheduled." });
        }

        await docClient.send(new UpdateCommand({
            TableName: "TestifyVouchers",
            Key: { voucherCode },
            UpdateExpression: "set #status = :s, scheduledTime = :st",
            ExpressionAttributeNames: { "#status": "status" },
            ExpressionAttributeValues: { ":s": "SCHEDULED", ":st": scheduledTime }
        }));

        const examLink = `https://testify.xetasolutions.in/exam-portal.html?testId=${voucher.testId}&voucher=${voucherCode}`;
        ...
        const candidateName = voucher.candidateName || 'Candidate';

        // Beautiful HTML Email Template for Scheduling
        const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
                ...
                <tr>
                    <td align="center">
                        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                            ...
                            <tr>
                                <td style="padding: 40px 30px;">
                                    <h2 style="margin-top: 0; color: #1e293b; font-size: 22px;">Hello ${candidateName},</h2>
                                    <p style="font-size: 16px; color: #475569; line-height: 1.6; margin-bottom: 25px;">
                                        Your premium mock exam has been successfully scheduled. Please mark your calendar!
                                    </p>
                                    
                                    <!-- Schedule Box -->
                                    <div style="background-color: #eff6ff; border-left: 4px solid #3b82f6; border-radius: 6px; padding: 20px; margin: 30px 0;">
                                        <p style="margin: 0 0 5px 0; font-size: 13px; color: #64748b; text-transform: uppercase; font-weight: bold; letter-spacing: 1px;">Scheduled Time</p>
                                        <p style="margin: 0; font-size: 18px; font-weight: 700; color: #1e3a8a;">${formattedDate}</p>
                                    </div>

                                    <!-- Proctoring Instructions Box -->
                                   <!-- Proctoring Instructions Box -->
<div style="background-color: #fdf2f8; border-left: 4px solid #db2777; border-radius: 6px; padding: 20px; margin: 30px 0;">
    
    <div style="text-align:center; margin-bottom:15px;">
        <h4 style="margin: 0 0 8px 0; font-size: 12px; color: #9d174d; text-transform: uppercase; font-weight: bold; letter-spacing: 1px;">
            Strictly Proctored By Proctorix
        </h4>

        <img src="https://res.cloudinary.com/dzuxwvamy/image/upload/v1778314086/Proctorix_Enterprise_AI_Proctoring_icyipk.png"
             alt="Proctorix"
             style="height: 40px; width:auto;">
    </div>

    <h3 style="margin: 0 0 15px 0; color: #901948; font-size: 16px;">
        Mandatory Exam Instructions:
    </h3>

    <ul style="margin: 0; padding-left: 20px; color: #831843; font-size: 14px; line-height: 1.8;">

        <li>
            <strong>Webcam & Microphone:</strong>
            Camera and microphone access are mandatory and must remain enabled throughout the examination.
        </li>

        <li>
            <strong>Identity Verification:</strong>
            You may be required to verify your identity before the examination begins.
        </li>

        <li>
            <strong>Face Visibility:</strong>
            Your entire face must remain clearly visible throughout the test. Avoid sitting in dark environments.
        </li>

        <li>
            <strong>Single Candidate Rule:</strong>
            Only the registered candidate is allowed in the testing area. Presence of another person will be flagged.
        </li>

        <li>
            <strong>Quiet Environment:</strong>
            Attend the exam from a distraction-free and noise-free location.
        </li>

        <li>
            <strong>No Mobile Phones:</strong>
            Mobile phones, smart watches, tablets, earphones, or any electronic devices are strictly prohibited.
        </li>

        <li>
            <strong>No External Materials:</strong>
            Books, notes, printed documents, AI tools, search engines, or external websites are not permitted unless explicitly allowed.
        </li>

        <li>
            <strong>No Tab Switching:</strong>
            Switching tabs, minimizing the browser, opening other applications, or leaving the exam screen will be recorded as a violation.
        </li>

        <li>
            <strong>Browser Permissions:</strong>
            Ensure camera, microphone, and browser permissions are granted before joining the exam.
        </li>

        <li>
            <strong>Stable Internet Connection:</strong>
            Maintain a reliable internet connection throughout the assessment to prevent interruptions.
        </li>

        <li>
            <strong>Screen Monitoring:</strong>
            Your screen activity, browser behavior, and examination session may be monitored and recorded.
        </li>

        <li>
            <strong>Multiple Face Detection:</strong>
            Detection of multiple faces or absence from the screen may result in warnings or termination of the assessment.
        </li>

        <li>
            <strong>Audio Monitoring:</strong>
            Unusual background conversations or suspicious audio activity may trigger proctoring alerts.
        </li>

        <li>
            <strong>No Impersonation:</strong>
            Any attempt to allow another individual to take the examination on your behalf will lead to immediate disqualification.
        </li>

        <li>
            <strong>Exam Integrity:</strong>
            Any cheating attempt, suspicious behavior, or policy violation may result in score cancellation and disqualification.
        </li>

        <li>
            <strong>Auto Submission:</strong>
            Multiple violations or critical integrity breaches can automatically submit your test and terminate your session.
        </li>

        <li>
            <strong>Early Login Recommended:</strong>
            Please join the exam portal at least 5–10 minutes before the scheduled start time for system checks.
        </li>

    </ul>
</div>
                                    
                                    <p style="font-size: 15px; color: #475569; line-height: 1.6; text-align: left; margin-bottom: 35px;">
                                        Please use the button below to access your secure exam portal. We recommend joining 5 minutes early to ensure a stable internet connection.
                                    </p>
                                    ...
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                                        <tr>
                                            <td align="center">
                                                <a href="${examLink}" style="background-color: #3b82f6; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: bold; font-size: 16px; display: inline-block; box-shadow: 0 4px 6px rgba(59, 130, 246, 0.25);">Go to Exam Portal</a>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                            ...
                            <tr>
                                <td align="center" style="background-color: #f8fafc; padding: 24px; border-top: 1px solid #edf2f7;">
                                    <p style="margin: 0; font-size: 14px; color: #94a3b8; line-height: 1.5;">
                                        Best regards,<br>
                                        <strong style="color: #64748b;">Credex by Testify</strong>
                                    </p>
                                </td>
                            </tr>
                        </table>
        ...
        const emailParams = {
            ...
            Content: {
                Simple: {
                    Subject: { Data: "Your Scheduled Mock Exam Link - Credex" },
                    Body: { 
                        Html: { Data: htmlContent, Charset: "UTF-8" },
                        Text: { Data: `Hello ${candidateName},\n\nYour premium mock exam has been successfully scheduled for ${formattedDate}.\n\nHere is your secure exam link:\n${examLink}\n\nPlease note: This exam is strictly proctored by Team Proctorix. Your camera and microphone must remain on, and tab switching is prohibited.\n\nPlease ensure you join on time and have a stable internet connection.\n\nBest regards,\nCredex by Testify`, Charset: "UTF-8" } 
                    }
                }
            }
        };
        ...
        await sesClient.send(new SendEmailCommand(emailParams));
        res.json({ message: "Test scheduled successfully! Check your email for the exam link." });
    } catch (error) {
        console.error("Scheduling/Email error:", error);
        res.status(500).json({ message: "Failed to schedule test or send exam link email." });
    }
});


app.post('/api/public/mock/submit', async (req, res) => {
    try {
        // ADDED idImage, liveImage, flaggedImage to the destructured body
        const { testId, voucherCode, userEmail, userName, answers, violationReason, idImage, liveImage, flaggedImage } = req.body;
        ...
        if (!voucher || voucher.assignedEmail !== userEmail || voucher.status === "COMPLETED") {
            return res.status(400).json({ message: "Invalid or already completed voucher." });
        }

        const { Item: testData } = await docClient.send(new GetCommand({
            TableName: "TestifyPublicTests",
            Key: { testId }
        }));
        ...
        testData.sections.forEach((sec, secIdx) => {
            ...
            sec.questions.forEach((q, qIdx) => {
                ...
                let isCorrect = false;
                if (q.type === 'single' && userAnswer === q.correct) isCorrect = true;
                else if (q.type === 'multi' && JSON.stringify(userAnswer?.sort()) === JSON.stringify(q.correct?.sort())) isCorrect = true;
                else if (q.type === 'fill' && q.correctAnswers?.includes(userAnswer?.toString().toLowerCase().trim())) isCorrect = true;
            ...
            sectionScores.push({
                ...
                score: secTotal > 0 ? Math.round((secCorrect / secTotal) * 100) : 0
            });
        });

        const scorePercentage = totalQuestions > 0 ? Math.round((correctAnswers / totalQuestions) * 100) : 0;
        const passingScore = 60; // Configurable
        const isQualified = scorePercentage >= passingScore && !violationReason; 

        const resultId = uuidv4();

        // Save Result to DynamoDB including the new image base64 strings
        await docClient.send(new PutCommand({
            TableName: "TestifyPublicResults", 
            Item: {
                resultId,
                ...
                answers,
                violationReason: violationReason || null,
                idImage: idImage || null,           // NEW: Saved ID Image
                liveImage: liveImage || null,       // NEW: Saved Live Start Frame
                flaggedImage: flaggedImage || null, // NEW: Saved AI Flag Frame
                isBlocked: false,                   // NEW: Default Block Status
                submittedAt: new Date().toISOString()
            }
        }));
        ...
        // Send Initial Pass/Fail Email (No certificate link yet)
        const subject = `Assessment Completed: ${testData.title}`;
        ...
        <div style="background-color: #f4f7f6; padding: 40px 20px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
            <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                ...
                <!-- Logo Header -->
                <div style="text-align: center; padding: 25px 20px; border-bottom: 1px solid #f0f0f0;">
                    <img src="https://res.cloudinary.com/dzuxwvamy/image/upload/v1780483593/Credex_szkzcq.png" alt="Credex Logo" style="max-height: 40px; width: auto;" />
                </div>
                
                <!-- Body Content -->
                <div style="padding: 30px 40px; color: #333333;">
                    <h2 style="margin-top: 0; color: #2c3e50; font-size: 22px;">Hello ${userName},</h2>
                    <p style="font-size: 16px; line-height: 1.6; color: #555555;">
                        You have successfully completed the assessment for <strong style="color: #2c3e50;">${testData.title}</strong>.
                    </p>
                    
                    <!-- Status Banner -->
                    <div style="margin: 25px 0; padding: 15px; background-color: ${isQualified ? '#e8f5e9' : '#ffebee'}; border-radius: 6px; text-align: center; border: 1px solid ${isQualified ? '#c8e6c9' : '#ffcdd2'};">
                        <h3 style="margin: 0; font-size: 20px; color: ${isQualified ? '#2e7d32' : '#c62828'};">
        ...
        if (violationReason) {
            htmlContent += `
                    <div style="margin-bottom: 20px; padding: 15px; border-left: 4px solid #d32f2f; background-color: #fff0f0; border-radius: 0 4px 4px 0;">
                        <p style="margin: 0; font-size: 15px; color: #d32f2f; line-height: 1.5;">
                            <strong>Proctoring Notice:</strong> Your exam was auto-submitted or flagged due to: <br/>
                            <span style="color: #b71c1c;">${violationReason}</span>
                        </p>
                    </div>
            `;
        } else {
            htmlContent += `
                    <p style="font-size: 15px; line-height: 1.6; color: #666666; margin-bottom: 20px;">
                        Your official scorecard and certificate (if applicable) will be released by the examiner shortly. You will receive an email once they are available.
                    </p>
            `;
        }
        ...
        htmlContent += `
                ...
                <!-- Footer -->
                <div style="background-color: #f9fafb; padding: 25px 20px; text-align: center; border-top: 1px solid #eeeeee; font-size: 13px; color: #777777;">
                    <p style="margin: 0 0 8px 0; font-size: 14px; font-weight: 600; color: #444444;">Credex by Testify</p>
                    <p style="margin: 0 0 5px 0;">Support: <a href="mailto:support@xetatech.in" style="color: #2980b9; text-decoration: none;">support@xetatech.in</a></p>
                </div>
            </div>
        </div>
        `;
        ...
        try {
            await sendEmailWithSES({ to: userEmail, subject, html: htmlContent });
        } catch(emailErr) {
            console.error("Email sending failed, but result saved:", emailErr);
        }

        res.status(200).json({ message: "Submitted successfully", score: scorePercentage, isQualified, resultId });
...
// --- 7. FETCH CERTIFICATE ---
app.get('/api/public/certificate/:certId', async (req, res) => {
    try {
        const { certId } = req.params;
        
        let result = await docClient.send(new GetCommand({
            TableName: "TestifyCertificates",
            Key: { certificateId: certId }
        }));

        // Fallback check just in case schema differs slightly
        if (!result.Item) {
        ...
        const dateObj = new Date(result.Item.issuedAt || result.Item.issueDate);
        const formattedDate = dateObj.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });

        res.status(200).json({
            certId: result.Item.certificateId || result.Item.certId,
            name: result.Item.userName || result.Item.studentName || 'Student',
            testName: result.Item.testTitle || result.Item.testName,
...
app.get('/api/public/scorecard/:resultId', async (req, res) => {
    try {
        const { resultId } = req.params;
        ...
        if (!result) {
            return res.status(404).json({ message: "Score card not found or invalid." });
        }
        ...
        res.status(200).json({
            ...result,
            testTitle: testData ? testData.title : 'Assessment',
            passingScore: 60 // Configurable standardi
        });

    } catch (err) {
        console.error("Fetch Scorecard Error:", err);
        res.status(500).json({ message: "Server error fetching score card." });
    }
});
// --- SME AUTH MIDDLEWARE ---
// const authMiddleware = (req, res, next) => {
//     const token = req.header('x-auth-token') || (req.headers['authorization'] ? req.headers['authorization'].split(' ')[1] : null);
...
// --- 8. GET SME TESTS ---
app.get('/api/sme/my-tests', authMiddleware, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'SME') {
            return res.status(403).json({ message: "Access denied. SME only." });
        }

        const userIdentifier = req.user.email; 
        
        // FIX: 'type' and 'domain' are reserved keywords in DynamoDB. 
        // We MUST use ExpressionAttributeNames (#typ, #dom) to map them safely.
        const params = {
            TableName: "TestifyPublicTests", 
            FilterExpression: "createdBy = :authorId", 
            ExpressionAttributeValues: { ":authorId": userIdentifier },
            ProjectionExpression: "testId, title, #typ, #dom, createdAt",
            ExpressionAttributeNames: { 
                "#typ": "type",
                "#dom": "domain" 
            } 
        ...
        res.json({ tests: data.Items || [] });

    } catch (error) {
        console.error("[Backend Error - my-tests]:", error);
        res.status(500).json({ message: "Server error fetching tests.", error: error.message });
    }
});

// --- 9. GET SPECIFIC TEST RESULTS ---
app.get('/api/sme/tests/:testId/results', authMiddleware, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'SME') return res.status(403).json({ message: "Access denied. SME only." });

        const { testId } = req.params;

        const checkTest = await docClient.send(new GetCommand({
            TableName: "TestifyPublicTests", Key: { testId: testId }
        }));
        
        if (!checkTest.Item || checkTest.Item.createdBy !== req.user.email) {
             return res.status(403).json({ message: "Unauthorized to view these results." });
        }

        const resultData = await docClient.send(new ScanCommand({
            TableName: "TestifyPublicResults",
            FilterExpression: "testId = :testId",
            ExpressionAttributeValues: { ":testId": testId }
        }));
        
        const formattedResults = (resultData.Items || []).map(item => ({
            resultId: item.resultId || "N/A",
            userName: item.userName || item.name || "Unknown",
            userEmail: item.userEmail || item.email || "N/A",
            score: item.score !== undefined ? item.score : 0,
            date: item.submittedAt || item.timestamp || new Date().toISOString(),
            violation: item.violationReason || null,
            isQualified: item.isQualified || false,
            scoreCardReleased: item.scoreCardReleased || false,
            certificateReleased: item.certificateReleased || false,
            // ADDED FIELDS FOR THE SME PORTAL EVIDENCE VIEWER
            idImage: item.idImage || null,
            liveImage: item.liveImage || null,
            flaggedImage: item.flaggedImage || null,
        ...
        console.error("[Backend Error - test results]:", error);
        res.status(500).json({ message: "Server error fetching test results." });
    }
});
app.post('/api/sme/results/:resultId/block', authMiddleware, async (req, res) => {
    try {
        // Ensure only SMEs can do this
        if (!req.user || req.user.role !== 'SME') return res.status(403).json({ message: "Access denied. SME only." });
        
        const { resultId } = req.params;

        // Fetch candidate result
        const { Item: result } = await docClient.send(new GetCommand({ 
            TableName: "TestifyPublicResults", 
            Key: { resultId } 
        }));
        ...
        // Ensure SME owns the test associated with this result
        const { Item: testData } = await docClient.send(new GetCommand({ 
            TableName: "TestifyPublicTests", 
            Key: { testId: result.testId } 
        }));
        if (!testData || testData.createdBy !== req.user.email) return res.status(403).json({ message: "Unauthorized" });

        // Update DB: Block Result and un-qualify candidate
        await docClient.send(new UpdateCommand({
            TableName: "TestifyPublicResults",
            Key: { resultId },
            UpdateExpression: "set isBlocked = :true, isQualified = :false",
            ExpressionAttributeValues: { 
                ":true": true, 
                ":false": false 
            }
        ...
        // Send Block Email using SES
        const subject = `Urgent Update: Exam Result Blocked - ${testData.title}`;
        const htmlContent = `
        <div style="background-color: #f4f7f6; padding: 40px 20px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
            <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; border-top: 6px solid #dc2626; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                ...
                <div style="padding: 30px 40px; color: #333333;">
                    <h2 style="margin-top: 0; color: #dc2626; font-size: 22px;">Exam Status Update</h2>
                    <p style="font-size: 16px; line-height: 1.6; color: #555555;">Dear ${result.userName},</p>
                    
                    <p style="font-size: 16px; line-height: 1.6; color: #555555;">
                        During the post-examination review process for the assessment <strong>${testData.title}</strong>, our examination panel discovered that the government ID or biometric evidence you uploaded does not match or was found to be invalid.
                    </p>
                    
                    <div style="margin: 25px 0; padding: 15px; background-color: #fef2f2; border-radius: 6px; border: 1px solid #fecaca;">
                        <p style="margin: 0; font-size: 15px; color: #b91c1c; font-weight: bold; text-align: center;">
                            As a result, your examination results have been permanently blocked.
                        </p>
                    </div>

                    <p style="font-size: 15px; line-height: 1.6; color: #666666; margin-bottom: 20px;">
                        If you believe this to be an error, please reach out to the administrative staff or your testing coordinator immediately to appeal this decision.
                    </p>
                </div>
                
                <div style="background-color: #f9fafb; padding: 25px 20px; text-align: center; border-top: 1px solid #eeeeee; font-size: 13px; color: #777777;">
                    <p style="margin: 0 0 8px 0; font-size: 14px; font-weight: 600; color: #444444;">Credex by Testify Integrity Team</p>
                    <p style="margin: 0 0 5px 0;">Support: <a href="mailto:support@xetatech.in" style="color: #2980b9; text-decoration: none;">support@xetatech.in</a></p>
                </div>
            </div>
        </div>
        ...
        await sendEmailWithSES({ to: result.userEmail, subject, html: htmlContent });

        res.json({ message: "Result blocked and candidate notified successfully." });

    } catch(err) {
        console.error("Block Document Error:", err);
        res.status(500).json({ message: "Server error blocking document." });
    }
});
// --- ADD THIS MISSING ENDPOINT FOR THE RESPONSE SHEET ---
app.get('/api/sme/results/:resultId/response-sheet', authMiddleware, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'SME') {
            return res.status(403).json({ message: "Access denied. SME only." });
        }
        ...
        // 3. Verify the SME actually owns the test
        if (test.createdBy !== req.user.email) {
            return res.status(403).json({ message: "Unauthorized to view these details." });
        }

        res.json({ result, test });
    } catch (error) {
        console.error("Error fetching response sheet:", error);
        res.status(500).json({ message: "Server error fetching response sheet." });
    }
});

app.get('/api/sme/public-tests/:testId', authMiddleware, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'SME') return res.status(403).json({ message: "Access denied. SME only." });
        
        const { Item } = await docClient.send(new GetCommand({
            TableName: "TestifyPublicTests",
            Key: { testId: req.params.testId }
        }));
...
// --- 11. UPDATE TEST ---
// --- 8.1 GET SINGLE PUBLIC TEST FOR EDITING ---
app.put('/api/sme/public-tests/:testId', async (req, res) => {
    try {
        const token = req.header('x-auth-token') || (req.headers['authorization'] ? req.headers['authorization'].split(' ')[1] : null);
        if (!token) {
            return res.status(401).json({ message: "No token, authorization denied." });
        ...
        const decoded = jwt.verify(token, process.env.JWT_SECRET || JWT_SECRET);
        const userIdentifier = decoded.id || decoded.email || decoded.user.email; 
        ...
        if (checkTest.Item.createdBy !== userIdentifier) {
        ...
        const updateParams = {
            TableName: "TestifyPublicTests",
            Key: { testId: testId },
            UpdateExpression: "set #typ = :type, title = :title, #dom = :domain, skills = :skills, description = :description, sections = :sections",
            ExpressionAttributeNames: {
                "#typ": "type",
                "#dom": "domain"
            },
        ...
        res.status(200).json({ message: "Test updated successfully", updatedTest: result.Attributes });
...
app.post('/api/sme/results/:resultId/release', authMiddleware, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'SME') return res.status(403).json({ message: "Access denied." });
        
        const { resultId } = req.params;
        const { docType } = req.body; 

        const { Item: result } = await docClient.send(new GetCommand({ TableName: "TestifyPublicResults", Key: { resultId } }));
        if (!result) return res.status(404).json({ message: "Result not found" });

        const { Item: testData } = await docClient.send(new GetCommand({ TableName: "TestifyPublicTests", Key: { testId: result.testId } }));
        if (!testData || testData.createdBy !== req.user.email) return res.status(403).json({ message: "Unauthorized" });
        ...
        if (docType === 'certificate') {
            if (!result.isQualified) return res.status(400).json({ message: "Candidate did not qualify for a certificate." });
            
            let certId = result.certId;
            if (!certId) {
                certId = `CERT-${uuidv4().substring(0,8).toUpperCase()}`;
                await docClient.send(new PutCommand({
                    TableName: "TestifyCertificates",
                    Item: {
                        certificateId: certId, certId: certId, testId: result.testId,
                        studentEmail: result.userEmail, userName: result.userName,
                        score: result.score, testTitle: testData.title, issuedAt: new Date().toISOString()
                    }
                }));
                updateExp = "set certId = :cid, certificateReleased = :true";
                expValues = { ":cid": certId, ":true": true };
            } else {
                updateExp = "set certificateReleased = :true";
                expValues = { ":true": true };
            }

            const certificateUrl = `https://testify.xetasolutions.in/credex-certificate.html?certId=${certId}`; 
            subject = `Your Verified Certificate for ${testData.title}`;
            ...
            <div style="background-color: #f4f7f6; padding: 40px 20px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                    ...
                    <!-- Body Content -->
                    <div style="padding: 30px 40px; color: #333333;">
                        <h2 style="margin-top: 0; color: #2c3e50; font-size: 22px;">Hello ${result.userName},</h2>
                        <p style="font-size: 16px; line-height: 1.6; color: #555555;">
                            Congratulations! Your verified certificate for <strong style="color: #2c3e50;">${testData.title}</strong> is now available.
                        </p>
                        
                        <div style="margin: 25px 0; padding: 15px; background-color: #f8f9fa; border-radius: 6px; text-align: center; border: 1px solid #e9ecef;">
                            <p style="margin: 0; font-size: 14px; color: #666;">Certificate ID</p>
                            <h3 style="margin: 5px 0 0 0; font-size: 18px; color: #2c3e50; font-family: monospace;">${certId}</h3>
                        </div>

                        <div style="text-align: center; margin-top: 30px;">
                            <a href="${certificateUrl}" style="background-color:#4f46e5; color:#ffffff; padding:12px 24px; text-decoration:none; border-radius:6px; display:inline-block; font-weight: bold; font-size: 15px;">View / Download Certificate</a>
                        </div>
                    </div>
                    
                    <!-- Footer -->
                    <div style="background-color: #f9fafb; padding: 20px; text-align: center; border-top: 1px solid #eeeeee;">
                        <h4 style="margin: 0 0 10px 0; color: #2c3e50; font-size: 14px;">Credex by Testify</h4>
                        <p style="margin: 5px 0; font-size: 13px; color: #666666;">
                            Support: <a href="mailto:support@xetatech.in" style="color: #2980b9; text-decoration: none;">support@xetatech.in</a>
                        </p>
                        <p style="margin: 5px 0 15px 0; font-size: 13px; color: #666666;">
                            <a href="https://testify.xetasolutions.in/exam-support" target="_blank" style="color: #2980b9; text-decoration: none;">testify.xetasolutions.in/exam-support</a>
                        </p>
                        <p style="margin: 0; font-size: 12px; color: #999999;">
                            This is an automated message from Credex, please do not reply to this email.
                        </p>
                    </div>
                </div>
            </div>
        ...
        } else if (docType === 'scorecard') {
            updateExp = "set scoreCardReleased = :true";
            expValues = { ":true": true };
            
            // Adjust this URL to wherever you host the new scorecard.html
            const scorecardUrl = `https://testify.xetasolutions.in/scorecard.html?resultId=${resultId}`;
            
            subject = `Your Score Report for ${testData.title}`;
            ...
            <div style="background-color: #f4f7f6; padding: 40px 20px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                    ...
                    <div style="padding: 30px 40px; color: #333333;">
                        <h2 style="margin-top: 0; color: #2c3e50; font-size: 22px;">Hello ${result.userName},</h2>
                        <p style="font-size: 16px; line-height: 1.6; color: #555555;">
                            Your official scorecard for the assessment <strong style="color: #2c3e50;">${testData.title}</strong> is now available.
                        </p>
                        <p style="font-size: 15px; line-height: 1.6; color: #666666; margin-bottom: 25px;">
                            You can view your detailed section-wise breakdown and download your report card using the link below:
                        </p>

                        <div style="text-align: center; margin-top: 30px;">
                            <a href="${scorecardUrl}" style="background-color:#059669; color:#ffffff; padding:12px 24px; text-decoration:none; border-radius:6px; display:inline-block; font-weight: bold; font-size: 15px;">View / Download Score Card</a>
                        </div>
                    </div>
        ...
        res.json({ message: `${docType === 'certificate' ? 'Certificate' : 'Score Card'} sent to candidate successfully!` });
...
app.delete('/api/sme/public-tests/:testId', authMiddleware, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'SME') return res.status(403).json({ message: "Access denied. SME only." });
...
// -------------------------------------------------------------------------
// 2. IDENTITY VERIFICATION (Amazon Rekognition CompareFaces)
// -------------------------------------------------------------------------
const PROCTOR_SESSION_TABLE = "TestifyProctorSessions"; 


// --- 1. KYC IDENTITY VERIFICATION ROUTE ---
app.post('/api/proctor/verify-identity', async (req, res) => {
    try {
        const { sessionId, idImageBase64, liveSnapshotBase64 } = req.body;

        if (!idImageBase64 || !liveSnapshotBase64) {
            return res.status(400).json({ success: false, message: "Missing image assets." });
        }

        // Convert base64 strings to Buffer arrays for Rekognition processing
        const idBuffer = Buffer.from(idImageBase64.replace(/^data:image\/\w+;base64,/, ""), 'base64');
        const liveBuffer = Buffer.from(liveSnapshotBase64.replace(/^data:image\/\w+;base64,/, ""), 'base64');

        const params = {
            SourceImage: { Bytes: idBuffer },
            TargetImage: { Bytes: liveBuffer },
            SimilarityThreshold: 85 // Requires 85% match confidence
        };
        ...
        if (response.FaceMatches && response.FaceMatches.length > 0) {
            res.json({ 
                success: true, 
                similarity: response.FaceMatches[0].Similarity.toFixed(2)
            });
        } else {
            res.status(403).json({ 
                success: false, 
                message: "Face mismatch. The live photo does not match the provided ID card." 
            });
        }
    } catch (err) {
        console.error("Rekognition Error:", err);
        res.status(500).json({ success: false, message: "AI Verification Error. Ensure clear lighting." });
    }
});
// -------------------------------------------------------------------------
// 4. LIVE FRAME ANALYSIS (Violation Detection)
// -------------------------------------------------------------------------
app.post('/api/proctor/analyze-frame', async (req, res) => {
    try {
        const { frameBase64 } = req.body;
        const imageBuffer = Buffer.from(frameBase64.replace(/^data:image\/\w+;base64,/, ""), 'base64');

        // Check for multiple faces
        const faceParams = { Image: { Bytes: imageBuffer } };
        const faceResponse = await rekognitionClient.send(new DetectFacesCommand(faceParams));

        if (faceResponse.FaceDetails.length > 1) {
            return res.json({ violation: true, reason: "Multiple faces detected in frame." });
        }
        if (faceResponse.FaceDetails.length === 0) {
            return res.json({ violation: true, reason: "No face detected in frame." });
        }

        // Optional: Check for prohibited objects (Phones, Books)
        const labelParams = { Image: { Bytes: imageBuffer }, MinConfidence: 80 };
        const labelResponse = await rekognitionClient.send(new DetectLabelsCommand(labelParams));
        
        const prohibitedObjects = ['Mobile Phone', 'Cell Phone', 'Book', 'Tablet Computer'];
        const foundProhibited = labelResponse.Labels.find(label => prohibitedObjects.includes(label.Name));

        if (foundProhibited) {
            return res.json({ violation: true, reason: `Prohibited object detected: ${foundProhibited.Name}` });
        }
...
app.post('/api/proctor/validate-voucher', async (req, res) => {
    try {
        ...
        if (!voucher || voucher.assignedEmail !== email) {
            return res.status(400).json({ success: false, message: "Invalid voucher code or email mismatch. Access Denied." });
        }
        
        if (voucher.status !== "ISSUED" && voucher.status !== "SCHEDULED") {
            return res.status(400).json({ success: false, message: "This voucher has already been utilized or is inactive." });
        }

        res.json({ 
            success: true, 
            testId: voucher.testId, 
            candidateName: voucher.candidateName || 'Candidate' 
        });

    } catch (error) {
        console.error("Voucher Validation Error:", error);
        res.status(500).json({ success: false, message: "Server error during voucher validation." });
    }
});

const { DetectLabelsCommand, DetectFacesCommand } = require("@aws-sdk/client-rekognition"); // Ensure these are imported

// --- ADD THIS MISSING ENDPOINT TO YOUR BACKEND ---
app.post('/api/proctor/verify-room-angle', async (req, res) => {
    try {
        const { sessionId, angle, liveSnapshotBase64 } = req.body;
        
        if (!liveSnapshotBase64) return res.status(400).json({ success: false, message: "No snapshot provided." });

        const liveBuffer = Buffer.from(liveSnapshotBase64.replace(/^data:image\/\w+;base64,/, ""), 'base64');

        // 1. Check for prohibited objects to ensure a clean room
        const labelResponse = await rekognitionClient.send(new DetectLabelsCommand({
            Image: { Bytes: liveBuffer },
            MinConfidence: 75
        }));
        
        const prohibited = ['Mobile Phone', 'Cell Phone', 'Tablet Computer', 'Electronics', 'Book', 'Paper'];
        const foundProhibited = labelResponse.Labels.find(l => prohibited.includes(l.Name));
        
        if (foundProhibited) {
            return res.json({ 
                success: false, 
                message: `Prohibited item detected in ${angle} view: ${foundProhibited.Name}. Please remove it and try again.` 
            });
        }

        // 2. Check for faces/secondary persons
        const faceResponse = await rekognitionClient.send(new DetectFacesCommand({
            Image: { Bytes: liveBuffer }
        }));

        if (faceResponse.FaceDetails.length > 0) {
            return res.json({ success: false, message: "A person was detected in the room. You must be alone." });
        }

        // Return success after passing AI structural checks
        res.json({ success: true, message: `${angle.charAt(0).toUpperCase() + angle.slice(1)} wall validated successfully.` });

    } catch (error) {
        console.error("Room Angle Verification Error:", error);
        res.status(500).json({ success: false, message: "Error validating room environment with AI." });
    }
});
