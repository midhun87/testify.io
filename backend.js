...
app.get('/api/public/certificate/:certId', async (req, res) => {
    try {
        ...
        // Attempt 1: Try with 'certId' as the primary key (Most likely correct based on your PutCommand)
        try {
            ...
            console.warn(`Attempt 1 failed for ${certId}. Mismatch on 'certId' schema.`);
        }

        // Attempt 2: If Attempt 1 failed or didn't find the item, gracefully try 'certificateId'
        if (!result || !result.Item) {
            try {
                ...
                console.warn(`Attempt 2 failed for ${certId}.`);
            }
        }

        // If both attempts yielded nothing, return a clean 404
        if (!result || !result.Item) {
            return res.status(404).json({ message: "Certificate not found or invalid." });
        }

        // Safely parse the date so it doesn't crash on undefined variables
        const rawDate = result.Item.issuedAt || result.Item.issueDate || new Date().toISOString();
...
app.post('/api/sme/results/:resultId/release', authMiddleware, async (req, res) => {
    try {
        ...
        if (docType === 'certificate') {
            ...
            const certificateUrl = `https://testify.xetasolutions.in/certificate.html?certId=${certId}`; 
