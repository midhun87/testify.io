...
app.post('/api/public/mock/submit', async (req, res) => {
    try {
        const { testId, voucherCode, userEmail, userName, answers, violationReason } = req.body;
        ...
        // Save Result (No certificate generation yet)
        await docClient.send(new PutCommand({
            TableName: "TestifyPublicResults", 
            Item: {
                resultId,
                ...
                type: 'mock',
                score: scorePercentage,
                sectionScores, // <--- SAVED SECTION SCORES
                isQualified,
                answers,
                violationReason: violationReason || null,
                submittedAt: new Date().toISOString()
        ...
        // Conditional violation or standard message
        if (violationReason) {
            htmlContent += `
        ...
        htmlContent += `
                ...
                <div style="background-color: #f9fafb; padding: 25px 20px; text-align: center; border-top: 1px solid #eeeeee; font-size: 13px; color: #777777;">
                    <p style="margin: 0 0 8px 0; font-size: 14px; font-weight: 600; color: #444444;">
                        Credex by Testify
                    </p>
...
app.get('/api/sme/tests/:testId/results', authMiddleware, async (req, res) => {
    try {
        ...
        const formattedResults = (resultData.Items || []).map(item => ({
            ...
            isQualified: item.isQualified || false, // <--- ADDED THIS
            scoreCardReleased: item.scoreCardReleased || false, // <--- ADDED THIS
            certificateReleased: item.certificateReleased || false // <--- ADDED THIS
        }));
...
app.post('/api/proctor/generate-upload-urls', async (req, res) => {
    try {
        const { sessionId, files } = req.body; // e.g., files: ['front', 'back', 'left', 'right']
        const urls = {};

        for (const fileType of files) {
            const fileKey = `proctor_assets/${sessionId}/${fileType}-${uuidv4()}.jpg`;
            ...
            urls[fileType] = { uploadUrl: url, key: fileKey };
        }
...
// -------------------------------------------------------------------------
// 3. MOBILE QR SYNC ENDPOINTS
// -------------------------------------------------------------------------

const activeRoomSessions = new Map(); // Temporary store to track mobile uploads
// --- 2. ROOM SYNC POLLING ROUTE (For Desktop) ---
app.get('/api/proctor/room-status/:sessionId', async (req, res) => {
    try {
        const { sessionId } = req.params;
        
        // Fetch session status from DynamoDB Table instead of Map
        const { Item: sessionData } = await docClient.send(new GetCommand({
            TableName: PROCTOR_SESSION_TABLE,
            Key: { sessionId }
        }));
        
        if (sessionData && sessionData.s3UploadsComplete) {
        ...
        res.status(500).json({ status: 'error', message: "Database polling failed." });
    }
});

// --- 3. MOBILE SYNC COMPLETION ROUTE (Called by mobile device) ---
// -------------------------------------------------------------------------
...
app.post('/api/proctor/verify-room-angle', async (req, res) => {
    try {
        ...
        // Check for prohibited objects to ensure a clean room
        ...
        // Check for faces/secondary persons
        ...
        if (faceResponse.FaceDetails.length > 0) {
            ...
            } else if (faceResponse.FaceDetails.length > 1) {
                return res.json({ success: false, message: "Multiple persons detected. You must be alone." });
            }
        }
...
app.post('/api/proctor/room-status/:sessionId', async (req, res) => {
    try {
        const { sessionId } = req.params;
        const { front, back, left, right } = req.body; 

        if (!front || !back || !left || !right) {
            return res.status(400).json({ success: false, message: "Missing one or more room angles." });
        }

        const assets = { front, back, left, right };
        
        // STRICT CHECK: Analyze uploaded images before allowing them to be accepted
        for (const [angle, base64Data] of Object.entries(assets)) {
            const buffer = Buffer.from(base64Data.replace(/^data:image\/\w+;base64,/, ""), 'base64');
            
            // Check if there are ANY persons in the room pictures
            const faceResponse = await rekognitionClient.send(new DetectFacesCommand({
                Image: { Bytes: buffer }
            }));
            ...
            if (faceResponse.FaceDetails.length > 0) {
                 return res.status(400).json({ 
                     success: false, 
                     message: `A person was detected in the ${angle} wall image. The room must be completely empty. Please ask them to leave and retake.` 
                 });
            }

            // Upload directly to S3 if valid
            await s3Client.send(new PutObjectCommand({
                Bucket: process.env.S3_BUCKET_NAME || 'hirewithusinterviewphotos',
                Key: `proctor-sessions/${sessionId}/room-${angle}.jpg`,
                Body: buffer,
                ContentType: 'image/jpeg'
            }));
        }

        // Mark the session as complete in DynamoDB only if all pass
        await docClient.send(new PutCommand({
            TableName: "TestifyProctorSessions",
            Item: { 
                sessionId: sessionId,
                s3UploadsComplete: true, 
                updatedAt: new Date().toISOString() 
            }
        }));
        ...
        console.error("Error uploading room status:", error);
        res.status(500).json({ success: false, message: "Failed to upload images and update session status." });
    }
});

app.post('/api/proctor/reverify-room-angle', async (req, res) => {
    try {
        const { sessionId, angle, liveSnapshotBase64 } = req.body;
        ...
        if (foundProhibited) {
            // Optional: Log this secondary violation to DynamoDB
            return res.json({ 
                success: false, 
                message: `Prohibited item detected during re-check in ${angle} view: ${foundProhibited.Name}. Please remove it and try again.` 
            });
        }
        ...
        if (faceResponse.FaceDetails.length > 0) {
            if (angle !== 'front') {
                ...
                return res.json({ success: false, message: "Multiple persons detected during re-check. You must be alone." });
            }
        }

        // Optional: Save the re-check images to S3 for enterprise audit trailing
        /*
        await s3Client.send(new PutObjectCommand({
            Bucket: process.env.S3_BUCKET_NAME,
            Key: `proctor-sessions/${sessionId}/mid-exam-recheck-${angle}-${Date.now()}.jpg`,
            Body: liveBuffer,
            ContentType: 'image/jpeg'
        }));
        */
        ...
        // Return success after passing AI structural checks
        res.json({ success: true, message: `${angle.charAt(0).toUpperCase() + angle.slice(1)} wall re-validated successfully.` });

    } catch (error) {
        console.error("Room Re-Verification Error:", error);
        res.status(500).json({ success: false, message: "Error validating room environment with AI during re-check." });
    }
});
