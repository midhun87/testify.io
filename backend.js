...
app.get('/api/public/reports/:email', async (req, res) => {
    try {
        ...
        // Extract student details. (Defaults are provided in case your DB doesn't store these directly in TestifyResults)
        ...
        // Map the results to include coding scores
        const publicReports = Items.map(item => {
            
            // Logic to handle coding score based on how you save it. 
            // If you have item.codingScore, it uses that. Otherwise defaults to 0 or extracts from item.result if it's nested there.
            let codingScore = item.codingScore || 0;
            if(item.result && typeof item.result.codingScore !== 'undefined') {
                codingScore = item.result.codingScore;
            }
