    ...
    try {
        ...
        if (!apiKey) {
            return res.status(403).json({ 
                error: { message: "GEMINI_API_KEY is not configured on the server. Please add it to your .env file." } 
            });
        }

        // We receive the prompt from the frontend
        ...
        // Securely call Gemini API from the backend
        ...
        // If Google returns an error, pass it back securely
        if (!response.ok) {
        ...
        res.status(500).json({ error: { message: 'Internal server error during AI generation' } });
    }
});
