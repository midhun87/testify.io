...
app.get('/api/public/test-details', authMiddleware, async (req, res) => {
    ...
    console.log(`[GET TEST DETAILS] Request for assignmentId: ${assignmentId}, email: ${candidateEmail}, isMock: ${isMockTest}`);

    try {
        const { Item: assignment } = await docClient.send(new GetCommand({ TableName: HIRING_ASSIGNMENTS_TABLE, Key: { assignmentId } }));

        // --- CHECK: Standard Hiring Flow (Not Mock) ---
        if (!isMockTest) {
            ...
            // ==================================================================
            // --- EMERGENCY FIX: TIME VALIDATION DISABLED ---
            // This prevents "Not Yet Active" errors due to Server Timezone differences.
            // ==================================================================
            /*
            const now = new Date();
            ...
            console.log(`[TIME CHECK] Current Server Time: ${now.toISOString()}`);
            console.log(`[TIME CHECK] Assignment Start Time: ${startTime.toISOString()}`);

            if (now < startTime) {
                 console.warn(`[GET TEST DETAILS] Test not yet active. (Now: ${now} < Start: ${startTime})`);
                 return res.status(403).json({ message: `This test link is not yet active. It starts at ${startTime.toLocaleString()}.` });
            }
            if (now > endTime) {
                 console.warn(`[GET TEST DETAILS] Test expired. (Now: ${now} > End: ${endTime})`);
                 return res.status(403).json({ message: 'This test link has expired.' });
            }
            */
            console.log(`[GET TEST DETAILS] Time validation BYPASSED for emergency access.`);
            // ==================================================================
           ...
           console.log(`[GET TEST DETAILS] No previous SUBMITTED result found.`);
            ...
            // --- MOCK TEST FLOW ---
            console.log(`[GET TEST DETAILS] MOCK TEST flow: Bypassing assignment, time, and submission checks.`);
        }
        
        // --- Fetch Actual Test Content (Common for both flows) ---
        let test;
        let targetTableName;
        let targetPkName;
        ...
        if (testType === 'coding') {
            ...
            test = Item;
             ...
             console.error(`[GET TEST DETAILS] Invalid testType '${testType}' in assignment.`);
             return res.status(500).json({ message: 'Internal Server Error: Invalid test type.' });
        }
        
        if (!test) {
            console.error(`[GET TEST DETAILS] Test definition ${testPkFromToken} not found in ${targetTableName}.`);
        ...
        processedTest.testType = testType; 
        processedTest.isMockTest = isMockTest || false;

        // Remove answers for aptitude tests
        ...
        // Simplify structure for coding tests
        ...
        console.log(`[GET TEST DETAILS] Success. Sending data.`);
