...
app.get('/api/public/certificate/:certId', async (req, res) => {
    try {
        ...
        // Fetch using ONLY the primary Partition Key (certificateId)
        ...
        // Catch the specific missing table/region error
        if (err.name === 'ResourceNotFoundException') {
            console.error("[AWS Error] DynamoDB Table missing or region mismatch: TestifyCertificates");
            return res.status(500).json({ message: "Database configuration error." });
        }

        // Catch all other server/AWS errors
