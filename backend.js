...
app.get('/api/public/reports/:email', async (req, res) => {
    try {
        ...
        const publicReports = Items.map(item => {
            ...
            // Extract the specific scores (Checks root level first, then inside item.result object)
            let codingScore = item.codingScore || (item.result && item.result.codingScore) || 0;
            let codeSpaceScore = item.codeSpaceScore || (item.result && item.result.codeSpaceScore) || 0;
            let sqlScore = item.sqlScore || (item.result && item.result.sqlScore) || 0;
