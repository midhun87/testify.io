...
app.get('/api/public/certificate/:certId', async (req, res) => {
    try {
        // Trim to ensure no hidden whitespace breaks the DynamoDB key match
        const certId = req.params.certId?.trim();
        console.log(`[PROD DEBUG] Fetching cert: ${certId}`);

        // Using the EXACT partition key from your AWS screenshot: certificateId
        ...
        const rawDate = item.issuedAt || item.issueDate || new Date().toISOString();
        ...
        res.status(200).json({
            certId: item.certificateId || item.certId || certId,
            name: item.userName || item.studentName || 'Student',
            testName: item.testTitle || item.testName || 'Assessment',
            score: item.score || 0,
            date: formattedDate
        });
        ...
        // URGENT FIX: Using status 400 instead of 500 here!
        // In production, Nginx/Apache often block 500 errors and show a default HTML page.
        // This prevents the frontend from reading our JSON error message. Status 400 bypasses this.
        res.status(400).json({ 
            message: `AWS Error: ${err.name} - ${err.message}` 
        });
    }
});
